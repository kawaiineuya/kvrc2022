...
double GlobalPlanner::getRisk(const Cell& cell) {
  // ROS_INFO("risk_cache_.find : %.2f, risk_cache_.end : %.2f ", risk_cache_.find(cell), risk_cache_.end());
  if (risk_cache_.find(cell) != risk_cache_.end()) {
    // ROS_INFO("risk_cache_.find");
    return risk_cache_[cell];
  }

  double risk = getSingleCellRisk(cell);
  // ROS_INFO("getSingleCellRisk(cell) : %.2f ",risk);
...
bool GlobalPlanner::getGlobalPath() {
  ...
  ROS_INFO("getRisk(t) : %.2f, max_cell_risk_ : %.2f ", getRisk(t), max_cell_risk_);
