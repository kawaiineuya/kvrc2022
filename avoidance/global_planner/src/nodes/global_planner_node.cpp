...
GlobalPlannerNode::GlobalPlannerNode(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private)
  ...
  // ROS_INFO("actual_path_.header.frame_id : %s", actual_path_.header.frame_id.c_str());
...
void GlobalPlannerNode::readParams() {
  ...
  // ROS_INFO("camera_topics : %s", camera_topics[0].c_str());
  if (!nh_.hasParam("camera_frame_id")) {
    nh_.setParam("camera_frame_id", "camera_link");
  } else {
    // ROS_INFO("camera_frame_id params");
    nh_.getParam("camera_frame_id", camera_frame_id_);
    // ROS_INFO("camera_frame_id : %s", camera_frame_id_.c_str());
  }


  // ROS_INFO("frame_id_ : %s", frame_id_.data.c_str());
...
void GlobalPlannerNode::dynamicReconfigureCallback(global_planner::GlobalPlannerNodeConfig& config, uint32_t level) {
  ...
  global_planner_.min_altitude_ = 0;
  // ROS_INFO("moveBaseSimpleCallback :%d",global_planner_.min_altitude_);
  ...
  global_planner_.max_cell_risk_ = 600;
...
void GlobalPlannerNode::clickedPointCallback(const geometry_msgs::PointStamped& msg) {
  // ROS_INFO("moveBaseSimpleCallbackmoveBaseSimpleCallbackmoveBaseSimpleCallbackmoveBaseSimpleCallbackmoveBaseSimpleCallback");
...
void GlobalPlannerNode::moveBaseSimpleCallback(const geometry_msgs::PoseStamped& msg) {
  ROS_INFO("moveBaseSimpleCallback");
