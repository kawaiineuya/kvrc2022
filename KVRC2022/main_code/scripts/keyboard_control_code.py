...
class robot():
    def __init__(self):
        ...
        self.goal_pose.pose.position.z = 0.8
        self.angle = 0
    ...
    def print_status(self):
        print(self.goal_pose.pose.position.x, self.goal_pose.pose.position.y,self.goal_pose.pose.position.z,self.angle)

    def input(self):
        key = self.getKey()
        if key == 'w' :
            self.goal_pose.pose.position.x +=1.6
            self.print_status()
        elif key == 'a' :
            ...
            quaternion = tf.transformations.quaternion_from_euler(0, 0, math.radians(self.angle))
            ...
            self.print_status()
            # orientation_list = [quaternion[0], quaternion[1], quaternion[2], quaternion[3]]
            # (self.roll, self.pitch, self.yaw) = euler_from_quaternion(orientation_list)
            # print(self.yaw)
        elif key == 'l' :
            self.angle -=90
